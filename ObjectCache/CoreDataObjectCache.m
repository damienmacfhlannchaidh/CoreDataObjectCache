...
#define ERROR_DOMAIN @"objectcache.errors"

@interface CoreDataObjectCacheCache ()
...
@implementation CoreDataObjectCacheCache

#pragma mark - Init

+ (id)cache
{
    return [CoreDataObjectCacheCache cacheFactoryWithCapacity:NSUIntegerMax];
}

+ (id)cacheWithCapacity:(NSUInteger)capacity
{
    return [CoreDataObjectCacheCache cacheFactoryWithCapacity:capacity];
}
...
+ (id)cacheFactoryWithCapacity:(NSUInteger)capacity
{
...
#endif
    static CoreDataObjectCacheCache *cache = nil;
    ...
    dispatch_once(&once_token, ^{
        ...
        NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"CoreDataObjectCacheCache" withExtension:@"momd"];
...
- (void)objectWithRequest:(NSURLRequest *)urlRequest modifyObject:(ObjectCacheModifyObjectBlock)modify success:(ObjectCacheSuccessBlock)success failure:(ObjectCacheFailureBlock)failure {
    ...
    if (cachedObject) {
        ...
        NSLog(@"cache miss: URL=%@", urlRequest.URL);
...
- (void)objectWithRequest:(NSURLRequest *)urlRequest success:(ObjectCacheSuccessBlock)success failure:(ObjectCacheFailureBlock)failure {
    ...
    [self objectWithRequest:urlRequest modifyObject:NULL success:success failure:failure];
}
...
- (void)removeObjectWithURL:(NSURL *)url success:(ObjectCacheSuccessBlock)success failure:(ObjectCacheFailureBlock)failure
{
    ...
    if (error) {
        NSLog(@"[OBJECT CACHE] Error occurred deleting object %@ from store. %@", url.absoluteString, error.localizedDescription);
        failure(error);
    } else {
...
- (void)resetObjectCache
{
    ...
    if (error) {
        NSLog(@"[OBJECT CACHE] Error occurred getting all objects in core data store. %@", error.localizedDescription);
        return;
    }
...
- (void)insertObjectIntoCoreDataWithURL:(NSURL *)url data:(NSData *)data expiryDate:(NSDate *)expiryDate;
{
    if (self.count+1 > self.capacity) {
        ...
        if (error) {
            NSLog(@"[OBJECT CACHE] Error occurred finding oldest object in store. %@", error.localizedDescription);
        } else {
            if (results.count == 1) {
                ...
                [_managedObjectContext save:&error];
                if (error) {
                    NSLog(@"[OBJECT CACHE] Error occurred deleting object %@ from store. %@", cachedObjectUrl, error.localizedDescription);
                } else {
    ...
    [_managedObjectContext save:&error];
    if (error) {
        NSLog(@"[OBJECT CACHE] Error occurred inserting object %@ into store. %@", url.absoluteString, error.localizedDescription);
    } else {
...
- (NSManagedObject *)objectFromCoreDataWithURL:(NSURL *)url
{
    ...
    // rolandg this needs more analysis!  was count == 1
...
{
    return [[[[NSFileManager defaultManager] URLsForDirectory:NSCachesDirectory inDomains:NSUserDomainMask] lastObject] URLByAppendingPathComponent:@"coredataobjectcache"];
}
